...
async function requestNoCatch(
  ...
  if (shouldUseAltRoute) {
    console.log(
      "Warning, sending " +
        type +
        " request to alternate host to fake error: " +
        host
    );
  } else {
    console.log(
  ...
  return fetch(`${host}${route}`, {
    ...
    if (response.ok) {
      if (route == "users/sign_in") {
        ...
        cookie = cookie.split(/, (?=\w+=)/) // Split by comma + new cookie token
        ...
        if (!cookie) {
          ...
          AsyncStorage.setItem("cookie", cookie.cookieString());
        }
      }
...
async function getRequest(
  ...
  if (shouldUseAltRoute) {
    console.log(
      "Warning, sending GET request to alternate host to fake error: " + host
    );
  }
  ...
  const url = params
    ...
    : `${host}${route}`;
  console.log("here's cookie" + cookie);
