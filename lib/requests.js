  ...
  return fetch(`${host}${route}`, {
    ...
    if (response.ok) {
      if (route == "users/sign_in") {
        ...
        let remember_cookie = cookie.split(/, (?=\w+=)/) // Split by comma + new cookie token
        .map(Cookie.parse) // Parse into cookie objects
        .find(cookie => cookie.key === 'remember_user_token'); // Find session token
        let calendar_cookie = cookie.split(/, (?=\w+=)/) // Split by comma + new cookie token
        .map(Cookie.parse) // Parse into cookie objects
        .find(cookie => cookie.key === '_calendar_session');
        if (!cookie) {
          console.warn(
            "Cookie could not be parsed for received header",
            response.headers.get("set-cookie")
          );
        } else {
          // AsyncStorage.setItem("cookie", remember_cookie.cookieString());
          if (remember_cookie){
            AsyncStorage.setItem("cookie", remember_cookie.cookieString());
          }
          else {
            AsyncStorage.setItem("cookie", calendar_cookie.cookieString());
          }
        }
 ...
 */
 async function getRequest(route, successFunc, errorFunc, params=null, shouldUseAltRoute=false) {
  ...
  const url = params
    ? `${host}${route}/?${queryString.stringify(params)}`
    : `${host}${route}`;
